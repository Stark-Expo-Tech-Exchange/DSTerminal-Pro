...
EDUCATION_TIPS = {
    ...
    1. [white]Updates Module Integration[/white] – Secure communication with official release repository
...
#  --------------------for updates below==================
    ...
    def check_updates(self):
        """Cross-platform cinematic updater with GitHub Releases support"""
        ...
        GITHUB_OWNER = "Stark-Expo-Tech-Exchange"
        GITHUB_REPO = "DSTerminal-Pro"

        CURRENT_VERSION = getattr(self, "VERSION", "2.1.327")
        ...
        HEADERS = {
            ...
            "User-Agent": f"DSTerminal-Updater/{CURRENT_VERSION}",
        }
        ...
        def hacker_animation():
            ...
            with console.status(
                "[bold cyan]🔐 ACCESSING UPDATE MODULES...[/]",
                spinner="dots"
            ):
                for _ in range(3):
        ...
        def satellite_scan():
            ...
            with Progress(
                ...
                TextColumn(
                    "[bold bright_blue]{task.description}",
                    justify="left"
                ),

                BarColumn(
                    bar_width=45,          # BIGGER BAR
                ...
                task = progress.add_task(
                    "Establishing secure encrypted update channel...",
                    total=100
                )
        ...
        def version_comparison_animation(current_ver, latest_ver):
            with Live(console=console, refresh_per_second=10, transient=True) as live:
                for i in range(1, 5):
                    ...
                    live.update(
                        Panel(
                            f"[bold cyan]Version Comparison[/bold cyan]\n\n"
                            f"[yellow]Current:[/] v{current_ver}\n"
                            f"[white]{bar:<30}[/white]\n\n"
        ...
        def get_system_info():
            """
            Returns:
            system_name, preferred_extensions
            """
            ...
            if system == "windows":
                ...
                return {
                    "os": "linux",
                    "extensions": [".deb", ".AppImage", ".rpm", ".sh"],
        ...
        def find_best_asset(assets, system_info):
            """
            Automatically selects correct installer
            for current OS
            """

            preferred_exts = system_info["extensions"]
            ...
            for ext in preferred_exts:
                for asset in assets:
                    ...
                    if any(k in name for k in keywords) and name.endswith(ext.lower()):
                        return asset
        ...
        def check_github_release():
            ...
            try:
                ...
                console.print(
                    "[cyan]🌐 Connecting to the Updates Module[/cyan]"
                )
                ...
                if response.status_code == 403:
                    console.print(
                        "[yellow]Updates Module API rate limit exceeded.[/yellow]"
                    )
                    return None

                if response.status_code != 200:
                    console.print(
                        f"[red]Updates Module API error: "
                ...
                if not latest_version:
                    console.print("[red]Invalid release data[/red]")
                    return None
                ...
                selected_asset = find_best_asset(assets, system_info)
                ...
                console.print(
                    f"[red]Network error:[/] {e}"
                )
                return None

            except Exception as e:
                console.print(
                    f"[red]Update parser error:[/] {e}"
                )
                return None
        ...
        def download_update(url, destination):
            ...
            try:
                ...
                console.print(
                    f"[green]✓ Download complete:[/] {destination}"
                )
            ...
            try:
                ...
                # ================= LINUX =================
                    ...
                    if installer_path.endswith(".deb"):
                        ...
                        console.print(
                            "[red]Unsupported Linux installer[/red]"
                        )
                        return False
                ...
                # ================= macOS =================
                    ...
                    if installer_path.endswith(".dmg"):
                        ...
                        console.print(
                            "[red]Unsupported macOS installer[/red]"
                        )
                        return False
                ...
                console.print(
                    f"[red]Installer launch failed:[/] {e}"
                )
        ...
        try:
            ...
            if not latest:
                console.print(
                    Panel(
                        "[red]Unable to check for updates[/red]",
                        border_style="red"
                    )
                )
                return False
            ...
            table.add_row(
                "Latest Version",
                f"[green]v{latest_version}[/green]"
            )

            table.add_row(
                "Installer",
                latest.get("asset_name") or "Not available"
            )

            if latest.get("asset_size"):

                size_mb = (
                    latest["asset_size"] / (1024 * 1024)
                )
            ...
            table.add_row(
                "Published",
                latest.get("published_at", "")[:10]
            )

            console.print(
                Panel(
                    table,
                    title="[bold yellow]🚀 UPDATES ARE AVAILABLE[/bold yellow]",
                    border_style="yellow",
                )
            )
...
# ================= RELEASE NOTES =================
            ...
            if notes:
                ...
                # Clean empty autogenerated GitHub note
                if notes.startswith("**Full Changelog**"):
                    notes = (
                        "# Release Information\n\n"
                        f"{notes}"
                    )
                ...
                console.print(
                    Panel(
                        Markdown(notes),
                        title="[bold cyan]📋 RELEASE NOTES[/bold cyan]",
                        border_style="cyan",
                        padding=(1, 2),
                    )
                )
            ...
            confirm = console.input(
                "\n[bold yellow]Download and install update? (Y/n): [/]"
            ).strip().lower()
            ...
            if not latest.get("download_url"):
                ...
                console.print(
                    Panel(
                        "[yellow]No compatible installer found "
                        "for your platform.[/yellow]\n\n"
                        f"For troubleshooting, contact the support team at support@dsterminal-mw.com",
                        # f"Release Page:\n{latest['url']}",
                        border_style="yellow",
                    )
                )
            ...
            run_now = console.input(
                "\n[bold green]Launch installer now? (Y/n): [/]"
            ).strip().lower()
            ...
            launched = install_update(installer_path)

            if launched:

                console.print(
                    Panel(
                        "[bold green]✅ Installer launched successfully[/bold green]\n\n"
                        "Follow the installation wizard to complete update.",
                        border_style="green",
                    )
                )
