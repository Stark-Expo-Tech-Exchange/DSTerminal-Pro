...
class DSTerminalMonitor(FileSystemEventHandler):
    ...
    def on_deleted(self, event):
        ...
        skip_patterns = ['*.tmp', '*.temp', 'cancellation-*', '*.db-journal', '*.db-wal', '*.db-shm']
        ...
        backup = self.db.find_backup_by_hash(file_hash)
        ...
        if backup:
            ...
            self.db.add_deletion_event(filepath, file_hash, backup['id'])
            self.logger.info(f"Deletion detected, backup exists: {filepath}")
        else:
            self.ui.display_notification(
                "DELETION DETECTED",
                f"⚠ {os.path.basename(filepath)}\nNo backup available",
            ...
            self.db.add_deletion_event(filepath, file_hash)
            self.logger.warning(f"File deleted without backup: {filepath}")
    ...
    def _quick_hash(self, filepath: str) -> str:
        """Quick hash for deletion detection"""
        if not os.path.exists(filepath):
