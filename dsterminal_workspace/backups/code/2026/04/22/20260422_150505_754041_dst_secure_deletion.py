...
class DSTerminalMonitor(FileSystemEventHandler):
    ...
    # Try to get file size before it's fully deleted
    try:
        file_size = os.path.getsize(filepath)
    except:
        file_size = 0
    
    # Search database for matching file
    cursor = self.db.conn.cursor()
    
    # First try: exact filename and similar size (within 10%)
    cursor.execute('''
        SELECT * FROM backups 
        WHERE filename = ? 
    ...
    # Second try: just filename (most recent)
    if not backup:
        cursor.execute('''
            SELECT * FROM backups 
            WHERE filename = ? 
